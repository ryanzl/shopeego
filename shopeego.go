...
func (s *ShopeeClient) makeV2Query(url string, b []byte) string {
	...
	q := ""
	if p.shopID > 0 {
		q = fmt.Sprintf("?partner_id=%d&shop_id=%d&timestamp=%d&sign=%s", p.partnerID, p.shopID, p.timestamp, sign)

	}else {
		q = fmt.Sprintf("?partner_id=%d&timestamp=%d&sign=%s", p.partnerID, p.timestamp, sign)
	}
	if s.accessToken != "" {
...
/*	req, err := http.NewRequest("POST", url, strings.NewReader(string(body)))
	...
	case ClientVersionV2:
		reqUrl := fmt.Sprintf("%s%s", url, s.makeV2Query(url, body))
